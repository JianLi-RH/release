...
### 3. CPOU Upgrade
- CPOU is an abbreviation for Control Plane Only Upgrade.
- CPOU is a kind of special upgrade, its file name contains both `cpou` and `upgrade`.
- Right now OpenShift only supports even version CPOU upgrades, so CPOU upgrade's target version must be an even version.
...
- Review [Job History](https://qe-private-deck-ci.apps.ci.l2s4.p1.openshiftapps.com/) regularly
- Adjust coverage based on customer version distribution trends
- Retire jobs for EOL versions following the one-release-after-EOL rule

## Common Pitfalls to Avoid

1. **DON'T** use disconnected profiles for chain upgrade jobs
2. **DON'T** add TP (TechPreview) jobs except for z-stream upgrades
3. **DON'T** forget to run `update-cron-entries.py` after changing job frequency
4. **DON'T** forget to sort jobs alphabetically in configuration files
5. **DON'T** add chain upgrades for EOL versions beyond one release after EOL
6. **DON'T** use odd version targets for CPOU upgrades (currently only even versions supported)

## Pre-Submission Checklist

Before submitting changes to upgrade configuration files:

- [ ] All jobs are sorted alphabetically by job name (`as` field)
- [ ] Job frequency matches upgrade type (f28 for Y/Chain/CPOU/Rollback, f60 for Z)
- [ ] Cron entries updated via `update-cron-entries.py`
- [ ] Image versions follow the rules (ansible/cli/tools vs upi-installer)
- [ ] Multi-arch coverage ≥65%
- [ ] Architecture-sensitive features copied across all architectures
- [ ] Chain upgrades cover all platforms
- [ ] No disconnected profiles in chain upgrades
- [ ] TP jobs only in z-stream upgrades
- [ ] `make update` executed successfully
